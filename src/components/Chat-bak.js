...
const Chat = ({ nombre }) => {
  ...
  return (
    <div className="h-600 max-h-600">
      <div id="messages" className = "flex flex-col min-h-0 space-y-4 p-3 overflow-y-auto scrollbar-thumb-blue scrollbar-thumb-rounded scrollbar-track-blue-lighter scrollbar-w-2">
        {mensajes.map((mensaje, i) => (
        <div key={i}>
        <React.Fragment>
          <div className={miNombre === mensaje.nombre ? 'flex items-end justify-end min-h-0' : 'flex items-start justify-start min-h-0' }>
            <div>
              <span className={miNombre === mensaje.nombre ? 
                'px-4 py-2 rounded-lg inline-block rounded-bl-none bg-green-400 text-white items-end' 
                :
